void registerSystemStates()
{
    ...
    registerState("Watch Face", "watch", [](){
        ...
        if (states[state].variant == 0) //actual watch face
        {
        ...
        else if (states[state].variant == 1) //ripoff control centre
        {
            ...
            if (!state_init) go_back_to_watch_face = false;
            ...
            if (dpad_left_active())
            {
                ...
                {
                    speaker_volume = std::max(speaker_volume - 1, 0);
                }
                else if (selected_widget == 1) // brightness
                {
                    screen_brightness = std::max(screen_brightness - 1, 0);
                    oled.sendCommand(0xC7, &screen_brightness, 1);
                ...
                {
                    torch_brightness = std::max(torch_brightness - 10, 0);
                    ledcWrite(0, torch_brightness);
                }
            }
            if (dpad_right_active())
            {
                ...
                {
                    speaker_volume = std::min(speaker_volume + 1, 21);
                }
                else if (selected_widget == 1) // brightness
                {
                    screen_brightness = std::min(screen_brightness + 1, 15);
                    oled.sendCommand(0xC7, &screen_brightness, 1);
                    preferences.begin("watch2", false);      //open watch II preferences in RW mode
                    preferences.putUChar("brightness", screen_brightness);
                    preferences.end();
                ...
                {
                    torch_brightness = std::min(torch_brightness + 10, 255);
                    ledcWrite(0, torch_brightness);
                }
            }
            if (dpad_up_active())
            ...
            if (!state_init || dpad_any_active())
            {
                ...
                if (selected_widget == 0) oled.fillRect(button_x - 2, button_y - 2, SCREEN_WIDTH - button_x, button_size + 4, BLACK);
                oled.drawRoundRect(button_x, button_y + (button_size / 4), SCREEN_WIDTH - spacing - button_x, button_size / 2, radius, WHITE);
                slider_x = button_x + ( speaker_volume * ( ( SCREEN_WIDTH - button_size - spacing - 2 - button_x ) / 21.0 ) );
                oled.fillRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, BLACK);
                oled.fillRoundRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, 0x2D50);                       //background thing
                outline_colour = (selected_widget == 0) ? WHITE : 0x2D50;                                           //determine outline colour
                oled.drawRoundRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, outline_colour);               //draw outline
                oled.drawBitmap(slider_x, button_y, small_icons["volume"].data(), button_size, button_size, WHITE);     //draw icon
                button_y += spacing + button_size;                                                                      //move cursor

                //draw brightness slider                                                                                yellow
                if (selected_widget == 1) oled.fillRect(button_x - 2, button_y - 2, SCREEN_WIDTH - button_x, button_size + 4, BLACK);
                oled.drawRoundRect(button_x, button_y + (button_size / 4), SCREEN_WIDTH - spacing - button_x, button_size / 2, radius, WHITE);
                slider_x = button_x + ( screen_brightness * ( ( SCREEN_WIDTH - button_size - spacing - 2 - button_x ) / 15.0 ) );
                oled.fillRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, BLACK);
                oled.fillRoundRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, 0xFEC0);                       //background thing
                outline_colour = (selected_widget == 1) ? WHITE : 0xFEC0;                                           //determine outline colour
                oled.drawRoundRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, outline_colour);               //draw outline
                oled.drawBitmap(slider_x, button_y, small_icons["sun"].data(), button_size, button_size, WHITE);        //draw icon
                button_y += spacing + button_size;                                                                      //move cursor

                //draw torch slider                                                                                    adafruit yellow
                if (selected_widget == 2) oled.fillRect(button_x - 2, button_y - 2, SCREEN_WIDTH - button_x, button_size + 4, BLACK);
                oled.drawRoundRect(button_x, button_y + (button_size / 4), SCREEN_WIDTH - spacing - button_x, button_size / 2, radius, WHITE);
                slider_x = button_x + ( torch_brightness * ( ( SCREEN_WIDTH - button_size - spacing - 2 - button_x ) / 255.0 ) );
                oled.fillRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, BLACK);
                oled.fillRoundRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, YELLOW);                       //background thing
                outline_colour = (selected_widget == 2) ? WHITE : YELLOW;                                           //determine outline colour
                oled.drawRoundRect(slider_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, outline_colour);               //draw outline
                oled.drawBitmap(slider_x, button_y, small_icons["torch_but_smaller"].data(), button_size, button_size, WHITE); //draw icon
                button_y += spacing + button_size;                                                                      //move cursor

                //draw wifi button
                outline_colour = (selected_widget == 3) ? 0x867D : WHITE;
                oled.drawRoundRect(button_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, outline_colour);
                oled.drawBitmap(button_x, button_y, small_icons["wifi"].data(), button_size, button_size, 0x867D);
                button_x += spacing + button_size;

                //draw bluetooth button  bluetooth blue
                background_colour = (bt_test) ? 0x041F : BLACK;
                oled.fillRoundRect(button_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, background_colour);
                outline_colour = ( !(selected_widget == 4) != !(bt_test) ) ? 0x041F : WHITE;
                oled.drawRoundRect(button_x - 2, button_y - 2, button_size + 4, button_size + 4, radius, outline_colour);
                oled.drawBitmap(button_x, button_y, small_icons["bluetooth"].data(), button_size, button_size, (bt_test) ? WHITE : 0x041F);
                button_x += spacing + button_size;
            ...
            if (dpad_down_active())
            {
                if (go_back_to_watch_face) switchState(state, 0);
            }
