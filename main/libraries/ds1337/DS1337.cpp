...
/**
 * \defgroup ds1337_ds1337_time_t Time Structure and Associated Methods
 */

/** 
 * \brief Set the values in a struct ds1337_time_t instance. 
 * 
 * \param time A pointer to a struct ds1337_time_t instance. 
 * \param year The year.
 ...
 */
void make_time(struct ds1337_time_t* time, uint8_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second) {
  time->year = year;
  time->month = month;
  time->day = day;
  time->hour = hour;
...
/**
 * \brief Output a string representation of a struct ds1337_time_t object.
 *
 ...
 */
void format_time_str(struct ds1337_time_t* time, char* buf) {
	// Year
	buf[0] = '2';
	if (time->year < 10) {
		buf[1] = '0';
 ...
 * 
 * \param time A pointer to a struct ds1337_time_t instance in which to store the time. 
 * 
 * \return 0 on success; otherwise an I2C error.
 */
uint8_t ds1337_read_time(struct ds1337_time_t* time) {
  uint8_t buf[7];
 ...
 * \brief Set the time. 
 * 
 * \param time A pointer to a struct ds1337_time_t instance containing the time to set. 
 * 
 * \return 0 on success; otherwise an I2C error.
 */
uint8_t ds1337_write_time(struct ds1337_time_t* time) {
  uint8_t buf[8];
